...
class WhatsAppSendMessageWizard(models.TransientModel):
    ...
    def send_message(self):
        """Send WhatsApp message with provider detection"""
        try:
                ...
                if self.message_type == 'text':
                    ...
                    if not media_base64:
                        return {
                            ...
                            'params': {
                                'title': 'Error',
                                'message': 'Invalid media file data',
                                'type': 'danger',
                            }
                        }
                    ...
                    if self.message_type == 'text':
                        ...
                        if not file_result.get('success'):
                            return {
                                ...
                                'params': {
                                    'title': 'Error',
                                    'message': f"Failed to upload file: {file_result.get('message', 'Unknown error')}",
                                    'type': 'danger',
                                }
                            }
            ...
            if result.get('success') or result.get('sent'):
                ...
                if self.group_id:
                    message_vals['group_id'] = self.group_id.id
                    # For WHAPI, also set chat_id
                    if config and config.provider == PROVIDER_WHAPI:
                        message_vals['chat_id'] = self.group_id.group_id
                else:
